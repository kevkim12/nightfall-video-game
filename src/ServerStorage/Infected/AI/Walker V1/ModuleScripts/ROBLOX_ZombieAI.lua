				...
				if object and object.Parent and object.Parent:FindFirstChild("HumanoidRootPart") and object.Health > 0 and object.WalkSpeed > 0 then				
					...
					if torso then
						...
						if distance <= configs["AggroRange"] then
							if object.Parent.Name == "Walker" then
								table.insert(zombies, object.Parent)
							elseif game.Players:GetPlayerFromCharacter(object.Parent) ~= nil then
								if game.Players:GetPlayerFromCharacter(object.Parent).Team == Infected then
									table.insert(zombies, object.Parent)
								else
...
--			end
	...
	local function handleHit(other)
		if canHit then
			if other and other.Parent and other.Parent:FindFirstChild("Humanoid") and game.Players:GetPlayerFromCharacter(other.Parent) ~= nil then
				if game.Players:GetPlayerFromCharacter(other.Parent).Team ~= Infected then
			...
			elseif other and other.Parent and other.Parent.Name ~= "Walker" and other.Parent:FindFirstChild("Humanoid") then
				local enemy = other.Parent						
				if enemy.Humanoid.WalkSpeed > 0 then
					enemy.Humanoid.Health = enemy.Humanoid.Health - configs["Damage"]
					canHit = false
